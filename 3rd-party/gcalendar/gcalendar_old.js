...
var background = { log:function(){}, updateBadge:function(){} };
...
// Moment.js language selection is weird; loading a language automatically
// switches to it as the active language. This is neither correct nor desired.
// Loading a lang_locale (e.g. 'en_US', 'pt_BR') works only if the specific
// language + locale combo is loaded, and does not fall back to the lang alone
// (e.g. 'en' or 'pt') by default.

// So, after loading all languages above, we first switch the default to 'en'.
// This establishes a failsafe default language that is no worse than not having
// any translations in the first place, and prevents the language being set to
// the last in the list of loaded languages (typically zh).

// Then, we try to load the lang_locale obtained from window.navigator.language.
// Immediately after attempting this, we check if it was successfully loaded by
// reading back the moment.lang(). If it silently failed (which it does when the
// language isn't available), we retry using just the lang (no locale, e.g.
// 'en' or 'pt').
 ...
 * @private
 */
scheduler.EVENTS_POLL_INTERVAL_MS_ = pollInterval // 60 * 60 * 1000;
